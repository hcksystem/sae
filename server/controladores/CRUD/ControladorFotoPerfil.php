...
class ControladorFotoPerfil extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO FotoPerfil (idPersona,foto) VALUES (?,?);";
      $parametros = array($fotoperfil->idPersona,$fotoperfil->foto);
   ...
   {
      $parametros = array($fotoperfil->idPersona,$fotoperfil->foto,$fotoperfil->id);
      $sql = "UPDATE FotoPerfil SET idPersona = ?,foto = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
