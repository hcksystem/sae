...
include_once('../entidades/CRUD/Aula.php');
class Controlador_aula extends Controlador_Base
{
   function crear($args)
   {
      $aula = new Aula($args["id"],$args["capacidad"],$args["descripcion"],$args["idTipoAula"]);
   ...
   {
      $registrosPorPagina = $args["registros_por_pagina"];
      $sql ="SELECT IF(ceil(count(*)/$registrosPorPagina)>0,ceil(count(*)/$registrosPorPagina),1) as 'paginas' FROM Aula;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      return $respuesta[0];
   }
