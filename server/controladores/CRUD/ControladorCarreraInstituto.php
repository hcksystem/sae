...
class ControladorCarreraInstituto extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO CarreraInstituto (idInstituto,idCarrera) VALUES (?,?);";
      $parametros = array($carrerainstituto->idInstituto,$carrerainstituto->idCarrera);
   ...
   {
      $parametros = array($carrerainstituto->idInstituto,$carrerainstituto->idCarrera,$carrerainstituto->id);
      $sql = "UPDATE CarreraInstituto SET idInstituto = ?,idCarrera = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
