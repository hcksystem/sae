...
include_once('../entidades/CRUD/Modalidad.php');
class Controlador_modalidad extends Controlador_Base
{
   function crear($args)
   {
   ...
   {
      $registrosPorPagina = $args["registros_por_pagina"];
      $sql ="SELECT IF(ceil(count(*)/$registrosPorPagina)>0,ceil(count(*)/$registrosPorPagina),1) as 'paginas' FROM Modalidad;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      return $respuesta[0];
   }
