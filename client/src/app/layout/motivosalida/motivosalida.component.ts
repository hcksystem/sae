...
import { MotivoSalida } from '../entidades/MotivoSalida';
...
@Component({
   selector: 'app-motivosalida',
   templateUrl: './motivosalida.component.html',
   styleUrls: ['./motivosalida.component.scss']
})

export class MotivoSalidaComponent implements OnInit {

   busy: Promise<any>;
   entidades: MotivoSalida[];
   entidadSeleccionada: MotivoSalida;
   pagina: 1;
   tamanoPagina: 20;
   esVisibleVentanaEdicion: boolean;

   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: MotivoSalidaService) {
   ...
   isValid(entidadPorEvaluar: MotivoSalida): boolean {
      return true;
   }
   ...
   crearEntidad(): MotivoSalida {
      const nuevoMotivoSalida = new MotivoSalida();
      nuevoMotivoSalida.id = 0;
      return nuevoMotivoSalida;
   }

   add(entidadNueva: MotivoSalida): void {
   ...
   update(entidadParaActualizar: MotivoSalida): void {
   ...
   delete(entidadParaBorrar: MotivoSalida): void {
   ...
   onSelect(entidadActual: MotivoSalida): void {
