...
import { PeriodoLectivo } from '../entidades/PeriodoLectivo';
...
@Component({
   selector: 'app-periodolectivo',
   templateUrl: './periodolectivo.component.html',
   styleUrls: ['./periodolectivo.component.scss']
})

export class PeriodoLectivoComponent implements OnInit {

   busy: Promise<any>;
   entidades: PeriodoLectivo[];
   entidadSeleccionada: PeriodoLectivo;
   pagina: 1;
   tamanoPagina: 20;
   esVisibleVentanaEdicion: boolean;

   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PeriodoLectivoService) {
   ...
   isValid(entidadPorEvaluar: PeriodoLectivo): boolean {
      return true;
   }
   ...
   crearEntidad(): PeriodoLectivo {
      const nuevoPeriodoLectivo = new PeriodoLectivo();
      nuevoPeriodoLectivo.id = 0;
      return nuevoPeriodoLectivo;
   }

   add(entidadNueva: PeriodoLectivo): void {
   ...
   update(entidadParaActualizar: PeriodoLectivo): void {
   ...
   delete(entidadParaBorrar: PeriodoLectivo): void {
   ...
   refresh(): void {
      this.entidades = PeriodoLectivo[0];
      this.entidadSeleccionada = this.crearEntidad();
      this.getPagina(1,5);
   ...
   onSelect(entidadActual: PeriodoLectivo): void {
