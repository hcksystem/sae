...
export class AsignaturaService {
    ...
    getAsignaturas(idDocente: number): Promise<AsignaturaDocente[]> {
        const url = `${this.urlBase + '/leerpordocente'}?idDocente=${idDocente}`;
        return this.http.get(url)
            .toPromise()
            .then(response => response.json() as AsignaturaDocente[])
            .catch(this.handleError);
    }
