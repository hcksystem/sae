...
@Component({
   selector: 'app-jornadacarrera',
   templateUrl: './jornadacarrera.component.html',
   styleUrls: ['./jornadacarrera.component.scss']
})

export class JornadaCarreraComponent implements OnInit {

   busy: Promise<any>;
   entidades: JornadaCarrera[];
   entidadSeleccionada: JornadaCarrera;
   pagina: 1;
   tamanoPagina: 20;
   esVisibleVentanaEdicion: boolean;

   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: JornadaCarreraService) {
   ...
   isValid(entidadPorEvaluar: JornadaCarrera): boolean {
      return true;
   }
   ...
   crearEntidad(): JornadaCarrera {
      const nuevoJornadaCarrera = new JornadaCarrera();
      nuevoJornadaCarrera.id = 0;
      return nuevoJornadaCarrera;
   }

   add(entidadNueva: JornadaCarrera): void {
   ...
   update(entidadParaActualizar: JornadaCarrera): void {
   ...
   delete(entidadParaBorrar: JornadaCarrera): void {
   ...
   refresh(): void {
      this.entidades = JornadaCarrera[0];
      this.entidadSeleccionada = this.crearEntidad();
      this.getAll();
   }
   ...
   onSelect(entidadActual: JornadaCarrera): void {
