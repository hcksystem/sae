...
import { Ocupacion } from '../entidades/Ocupacion';
...
export class OcupacionComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: OcupacionService) {
   ...
   isValid(entidadPorEvaluar: Ocupacion): boolean {
      return true;
   }
   ...
   crearEntidad(): Ocupacion {
      const nuevoOcupacion = new Ocupacion();
      nuevoOcupacion.id = 0;
      return nuevoOcupacion;
   }

   add(entidadNueva: Ocupacion): void {
   ...
   update(entidadParaActualizar: Ocupacion): void {
   ...
   delete(entidadParaBorrar: Ocupacion): void {
   ...
   onSelect(entidadActual: Ocupacion): void {
