...
import { DetalleNotas } from '../entidades/DetalleNotas';
...
@Component({
   selector: 'app-detallenotas',
   templateUrl: './detallenotas.component.html',
   styleUrls: ['./detallenotas.component.scss']
})

export class DetalleNotasComponent implements OnInit {

   busy: Promise<any>;
   entidades: DetalleNotas[];
   ...
   isValid(entidadPorEvaluar: DetalleNotas): boolean {
      return true;
   }
   ...
   crearEntidad(): DetalleNotas {
      const nuevoDetalleNotas = new DetalleNotas();
      nuevoDetalleNotas.id = 0;
      return nuevoDetalleNotas;
   }

   add(entidadNueva: DetalleNotas): void {
   ...
   update(entidadParaActualizar: DetalleNotas): void {
   ...
   delete(entidadParaBorrar: DetalleNotas): void {
   ...
   onSelect(entidadActual: DetalleNotas): void {
