...
import { EstadoCivil } from '../entidades/EstadoCivil';
...
export class EstadoCivilComponent implements OnInit {
   ...
   busy: Promise<any>;
   entidades: EstadoCivil[];
   entidadSeleccionada: EstadoCivil;
   pagina: 1;
   tamanoPagina: 20;
   esVisibleVentanaEdicion: boolean;

   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EstadoCivilService) {
   ...
   isValid(entidadPorEvaluar: EstadoCivil): boolean {
      return true;
   }
   ...
   crearEntidad(): EstadoCivil {
      const nuevoEstadoCivil = new EstadoCivil();
      nuevoEstadoCivil.id = 0;
      return nuevoEstadoCivil;
   }

   add(entidadNueva: EstadoCivil): void {
   ...
   update(entidadParaActualizar: EstadoCivil): void {
   ...
   delete(entidadParaBorrar: EstadoCivil): void {
   ...
   onSelect(entidadActual: EstadoCivil): void {
