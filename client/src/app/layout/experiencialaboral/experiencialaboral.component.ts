...
export class ExperienciaLaboralComponent implements OnInit {
   ...
   busy: Promise<any>;
   entidades: ExperienciaLaboral[];
   entidadSeleccionada: ExperienciaLaboral;
   pagina: 1;
   tamanoPagina: 20;
   esVisibleVentanaEdicion: boolean;

   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ExperienciaLaboralService) {
   ...
   isValid(entidadPorEvaluar: ExperienciaLaboral): boolean {
      return true;
   }
   ...
   crearEntidad(): ExperienciaLaboral {
      const nuevoExperienciaLaboral = new ExperienciaLaboral();
      nuevoExperienciaLaboral.id = 0;
      return nuevoExperienciaLaboral;
   }

   add(entidadNueva: ExperienciaLaboral): void {
   ...
   update(entidadParaActualizar: ExperienciaLaboral): void {
   ...
   delete(entidadParaBorrar: ExperienciaLaboral): void {
   ...
   refresh(): void {
      this.entidades = ExperienciaLaboral[0];
      this.entidadSeleccionada = this.crearEntidad();
      this.getPagina(1,5);
   }
   ...
   onSelect(entidadActual: ExperienciaLaboral): void {
