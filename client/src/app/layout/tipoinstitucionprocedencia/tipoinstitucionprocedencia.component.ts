...
import { TipoInstitucionProcedencia } from '../entidades/TipoInstitucionProcedencia';
...
@Component({
   selector: 'app-tipoinstitucionprocedencia',
   templateUrl: './tipoinstitucionprocedencia.component.html',
   styleUrls: ['./tipoinstitucionprocedencia.component.scss']
})

export class TipoInstitucionProcedenciaComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoInstitucionProcedencia[];
   entidadSeleccionada: TipoInstitucionProcedencia;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoInstitucionProcedenciaService) {
   ...
   isValid(entidadPorEvaluar: TipoInstitucionProcedencia): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoInstitucionProcedencia {
      const nuevoTipoInstitucionProcedencia = new TipoInstitucionProcedencia();
      nuevoTipoInstitucionProcedencia.id = 0;
      return nuevoTipoInstitucionProcedencia;
   }

   add(entidadNueva: TipoInstitucionProcedencia): void {
   ...
   update(entidadParaActualizar: TipoInstitucionProcedencia): void {
   ...
   delete(entidadParaBorrar: TipoInstitucionProcedencia): void {
   ...
   refresh(): void {
      ...
      this.entidades = TipoInstitucionProcedencia[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: TipoInstitucionProcedencia): void {
