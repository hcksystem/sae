...
import { EstadoSolicitud } from '../entidades/EstadoSolicitud';
...
@Component({
   ...
   styleUrls: ['./estadosolicitud.component.scss']
})

export class EstadoSolicitudComponent implements OnInit {

   busy: Promise<any>;
   entidades: EstadoSolicitud[];
   entidadSeleccionada: EstadoSolicitud;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EstadoSolicitudService) {
   ...
   isValid(entidadPorEvaluar: EstadoSolicitud): boolean {
      return true;
   }
   ...
   crearEntidad(): EstadoSolicitud {
      const nuevoEstadoSolicitud = new EstadoSolicitud();
      nuevoEstadoSolicitud.id = 0;
      return nuevoEstadoSolicitud;
   }

   add(entidadNueva: EstadoSolicitud): void {
   ...
   update(entidadParaActualizar: EstadoSolicitud): void {
   ...
   delete(entidadParaBorrar: EstadoSolicitud): void {
   ...
   refresh(): void {
      this.entidades = EstadoSolicitud[0];
      this.entidadSeleccionada = this.crearEntidad();
      this.getPagina(1,5);
   ...
   onSelect(entidadActual: EstadoSolicitud): void {
