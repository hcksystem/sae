...
import { TipoEducacionContinua } from '../entidades/TipoEducacionContinua';
...
@Component({
   selector: 'app-tipoeducacioncontinua',
   templateUrl: './tipoeducacioncontinua.component.html',
   styleUrls: ['./tipoeducacioncontinua.component.scss']
})

export class TipoEducacionContinuaComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoEducacionContinua[];
   entidadSeleccionada: TipoEducacionContinua;
   pagina: 1;
   tamanoPagina: 20;
   ...
   isValid(entidadPorEvaluar: TipoEducacionContinua): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoEducacionContinua {
      const nuevoTipoEducacionContinua = new TipoEducacionContinua();
      nuevoTipoEducacionContinua.id = 0;
      return nuevoTipoEducacionContinua;
   }

   add(entidadNueva: TipoEducacionContinua): void {
   ...
   update(entidadParaActualizar: TipoEducacionContinua): void {
   ...
   delete(entidadParaBorrar: TipoEducacionContinua): void {
   ...
   refresh(): void {
      this.entidades = TipoEducacionContinua[0];
      this.entidadSeleccionada = this.crearEntidad();
      this.getAll();
   }
   ...
   onSelect(entidadActual: TipoEducacionContinua): void {
