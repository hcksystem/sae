...
export class CupoComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CupoService) {
   ...
   isValid(entidadPorEvaluar: Cupo): boolean {
      return true;
   }
   ...
   crearEntidad(): Cupo {
      const nuevoCupo = new Cupo();
      nuevoCupo.id = 0;
      return nuevoCupo;
   }

   add(entidadNueva: Cupo): void {
   ...
   update(entidadParaActualizar: Cupo): void {
   ...
   delete(entidadParaBorrar: Cupo): void {
   ...
   onSelect(entidadActual: Cupo): void {
