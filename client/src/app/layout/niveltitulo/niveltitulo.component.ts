...
@Component({
   selector: 'app-niveltitulo',
   templateUrl: './niveltitulo.component.html',
   styleUrls: ['./niveltitulo.component.scss']
})

export class NivelTituloComponent implements OnInit {

   busy: Promise<any>;
   entidades: NivelTitulo[];
   entidadSeleccionada: NivelTitulo;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: NivelTituloService) {
   ...
   isValid(entidadPorEvaluar: NivelTitulo): boolean {
      return true;
   }
   ...
   crearEntidad(): NivelTitulo {
      const nuevoNivelTitulo = new NivelTitulo();
      nuevoNivelTitulo.id = 0;
      return nuevoNivelTitulo;
   }

   add(entidadNueva: NivelTitulo): void {
   ...
   update(entidadParaActualizar: NivelTitulo): void {
   ...
   delete(entidadParaBorrar: NivelTitulo): void {
   ...
   onSelect(entidadActual: NivelTitulo): void {
