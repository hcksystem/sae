...
export class DatosEstudianteService {
   ...
   private urlBase = environment.apiUrl + 'datosestudiante';
   ...
   getAll(): Promise<DatosEstudiante[]> {
      return this.http.get(this.urlBase+'/leer').toPromise().then(response=>response.json() as DatosEstudiante[]).catch(this.handleError);
   }
   ...
   getFiltrado(columna: string, tipoFiltro: string, filtro: string): Promise<DatosEstudiante[]> {
      return this.http.get(this.urlBase+'/leer_filtrado' + '?columna=' + columna + '&tipo_filtro=' + tipoFiltro + '&filtro=' + filtro).toPromise().then(response=>response.json() as DatosEstudiante[]).catch(this.handleError);
   }
   ...
   get(id: number): Promise<DatosEstudiante> {
      ...
      return this.http.get(url).toPromise().then(response=>response.json() as DatosEstudiante).catch(this.handleError);
   }
   ...
   create(entidadTransporte: DatosEstudiante): Promise<boolean> {
   ...
   update(entidadTransporte: DatosEstudiante): Promise<boolean> {
