...
import { DatosEstudiante } from '../../entidades/CRUD/DatosEstudiante';
...
@Component({
   selector: 'app-datosestudiante',
   templateUrl: './datosestudiante.component.html',
   styleUrls: ['./datosestudiante.component.scss']
})

export class DatosEstudianteComponent implements OnInit {

   busy: Promise<any>;
   entidades: DatosEstudiante[];
   entidadSeleccionada: DatosEstudiante;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: DatosEstudianteService) {
   ...
   isValid(entidadPorEvaluar: DatosEstudiante): boolean {
      return true;
   }
   ...
   crearEntidad(): DatosEstudiante {
      const nuevoDatosEstudiante = new DatosEstudiante();
      nuevoDatosEstudiante.id = 0;
      return nuevoDatosEstudiante;
   }

   add(entidadNueva: DatosEstudiante): void {
   ...
   update(entidadParaActualizar: DatosEstudiante): void {
   ...
   delete(entidadParaBorrar: DatosEstudiante): void {
   ...
   refresh(): void {
      ...
      this.entidades = DatosEstudiante[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: DatosEstudiante): void {
