...
@Component({
   selector: 'app-categorianota',
   templateUrl: './categorianota.component.html',
   styleUrls: ['./categorianota.component.scss']
})

export class CategoriaNotaComponent implements OnInit {

   busy: Promise<any>;
   entidades: CategoriaNota[];
   entidadSeleccionada: CategoriaNota;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CategoriaNotaService) {
   ...
   isValid(entidadPorEvaluar: CategoriaNota): boolean {
      return true;
   }
   ...
   crearEntidad(): CategoriaNota {
      const nuevoCategoriaNota = new CategoriaNota();
      nuevoCategoriaNota.id = 0;
      return nuevoCategoriaNota;
   }

   add(entidadNueva: CategoriaNota): void {
   ...
   update(entidadParaActualizar: CategoriaNota): void {
   ...
   delete(entidadParaBorrar: CategoriaNota): void {
   ...
   refresh(): void {
      ...
      this.entidades = CategoriaNota[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CategoriaNota): void {
