...
@Component({
   selector: 'app-docenteasignatura',
   templateUrl: './docenteasignatura.component.html',
   styleUrls: ['./docenteasignatura.component.scss']
})

export class DocenteAsignaturaComponent implements OnInit {

   busy: Promise<any>;
   entidades: DocenteAsignatura[];
   entidadSeleccionada: DocenteAsignatura;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: DocenteAsignaturaService) {
   ...
   isValid(entidadPorEvaluar: DocenteAsignatura): boolean {
      return true;
   }
   ...
   crearEntidad(): DocenteAsignatura {
      const nuevoDocenteAsignatura = new DocenteAsignatura();
      nuevoDocenteAsignatura.id = 0;
      return nuevoDocenteAsignatura;
   }

   add(entidadNueva: DocenteAsignatura): void {
   ...
   update(entidadParaActualizar: DocenteAsignatura): void {
   ...
   delete(entidadParaBorrar: DocenteAsignatura): void {
   ...
   onSelect(entidadActual: DocenteAsignatura): void {
