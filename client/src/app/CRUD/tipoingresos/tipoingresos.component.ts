...
import { TipoIngresos } from '../../entidades/TipoIngresos';
...
@Component({
   selector: 'app-tipoingresos',
   templateUrl: './tipoingresos.component.html',
   styleUrls: ['./tipoingresos.component.scss']
})

export class TipoIngresosComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoIngresos[];
   entidadSeleccionada: TipoIngresos;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoIngresosService) {
   ...
   isValid(entidadPorEvaluar: TipoIngresos): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoIngresos {
      const nuevoTipoIngresos = new TipoIngresos();
      nuevoTipoIngresos.id = 0;
      return nuevoTipoIngresos;
   }

   add(entidadNueva: TipoIngresos): void {
   ...
   update(entidadParaActualizar: TipoIngresos): void {
   ...
   delete(entidadParaBorrar: TipoIngresos): void {
   ...
   onSelect(entidadActual: TipoIngresos): void {
