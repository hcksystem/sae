...
export class MallaComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: MallaService) {
   ...
   isValid(entidadPorEvaluar: Malla): boolean {
      return true;
   }
   ...
   crearEntidad(): Malla {
      const nuevoMalla = new Malla();
      nuevoMalla.id = 0;
      return nuevoMalla;
   }

   add(entidadNueva: Malla): void {
   ...
   update(entidadParaActualizar: Malla): void {
   ...
   delete(entidadParaBorrar: Malla): void {
   ...
   onSelect(entidadActual: Malla): void {
