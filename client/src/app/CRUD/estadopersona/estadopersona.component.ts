...
import { EstadoPersona } from '../../entidades/CRUD/EstadoPersona';
...
@Component({
   selector: 'app-estadopersona',
   templateUrl: './estadopersona.component.html',
   styleUrls: ['./estadopersona.component.scss']
})

export class EstadoPersonaComponent implements OnInit {

   busy: Promise<any>;
   entidades: EstadoPersona[];
   entidadSeleccionada: EstadoPersona;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EstadoPersonaService) {
   ...
   isValid(entidadPorEvaluar: EstadoPersona): boolean {
      return true;
   }
   ...
   crearEntidad(): EstadoPersona {
      const nuevoEstadoPersona = new EstadoPersona();
      nuevoEstadoPersona.id = 0;
      return nuevoEstadoPersona;
   }

   add(entidadNueva: EstadoPersona): void {
   ...
   update(entidadParaActualizar: EstadoPersona): void {
   ...
   delete(entidadParaBorrar: EstadoPersona): void {
   ...
   refresh(): void {
      ...
      this.entidades = EstadoPersona[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: EstadoPersona): void {
