...
@Component({
   ...
   styleUrls: ['./tiposangre.component.scss']
})

export class TipoSangreComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoSangre[];
   ...
   isValid(entidadPorEvaluar: TipoSangre): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoSangre {
      const nuevoTipoSangre = new TipoSangre();
      nuevoTipoSangre.id = 0;
      return nuevoTipoSangre;
   }

   add(entidadNueva: TipoSangre): void {
   ...
   update(entidadParaActualizar: TipoSangre): void {
   ...
   delete(entidadParaBorrar: TipoSangre): void {
   ...
   onSelect(entidadActual: TipoSangre): void {
