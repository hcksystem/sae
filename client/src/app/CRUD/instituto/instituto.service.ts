...
import { Instituto } from '../../entidades/CRUD/Instituto';

@Injectable()

export class InstitutoService {
   private headers = new Headers({ 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
   private urlBase = environment.apiUrl + 'instituto';
   ...
   getAll(): Promise<Instituto[]> {
      return this.http.get(this.urlBase+'/leer').toPromise().then(response=>response.json() as Instituto[]).catch(this.handleError);
   }
   ...
   getFiltrado(columna: string, tipoFiltro: string, filtro: string): Promise<Instituto[]> {
      return this.http.get(this.urlBase+'/leer_filtrado' + '?columna=' + columna + '&tipo_filtro=' + tipoFiltro + '&filtro=' + filtro).toPromise().then(response=>response.json() as Instituto[]).catch(this.handleError);
   }
   ...
   get(id: number): Promise<Instituto> {
      ...
      return this.http.get(url).toPromise().then(response=>response.json() as Instituto).catch(this.handleError);
   }
   ...
   create(entidadTransporte: Instituto): Promise<boolean> {
   ...
   update(entidadTransporte: Instituto): Promise<boolean> {
