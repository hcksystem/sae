...
import { TipoRequisito } from '../../entidades/CRUD/TipoRequisito';
...
@Component({
   selector: 'app-tiporequisito',
   templateUrl: './tiporequisito.component.html',
   styleUrls: ['./tiporequisito.component.scss']
})

export class TipoRequisitoComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoRequisito[];
   entidadSeleccionada: TipoRequisito;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoRequisitoService) {
   ...
   isValid(entidadPorEvaluar: TipoRequisito): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoRequisito {
      const nuevoTipoRequisito = new TipoRequisito();
      nuevoTipoRequisito.id = 0;
      return nuevoTipoRequisito;
   }

   add(entidadNueva: TipoRequisito): void {
   ...
   update(entidadParaActualizar: TipoRequisito): void {
   ...
   delete(entidadParaBorrar: TipoRequisito): void {
   ...
   refresh(): void {
      ...
      this.entidades = TipoRequisito[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: TipoRequisito): void {
