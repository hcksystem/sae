...
export class PeriodoLectivoService {
   ...
   private urlBase = environment.apiUrl + 'periodolectivo';
   ...
   getAll(): Promise<PeriodoLectivo[]> {
      return this.http.get(this.urlBase+'/leer').toPromise().then(response=>response.json() as PeriodoLectivo[]).catch(this.handleError);
   }
   ...
   getFiltrado(columna: string, tipoFiltro: string, filtro: string): Promise<PeriodoLectivo[]> {
      return this.http.get(this.urlBase+'/leer_filtrado' + '?columna=' + columna + '&tipo_filtro=' + tipoFiltro + '&filtro=' + filtro).toPromise().then(response=>response.json() as PeriodoLectivo[]).catch(this.handleError);
   }
   ...
   get(id: number): Promise<PeriodoLectivo> {
      ...
      return this.http.get(url).toPromise().then(response=>(response.json() as PeriodoLectivo[])[0]).catch(this.handleError);
   }

   remove(id: number): Promise<boolean> {
      const url = `${this.urlBase+'/borrar'}?id=${id}`;
      return this.http.get(url).toPromise().then(response=>response.json() as PeriodoLectivo).catch(this.handleError);
   }

   create(entidadTransporte: PeriodoLectivo): Promise<boolean> {
   ...
   update(entidadTransporte: PeriodoLectivo): Promise<boolean> {
